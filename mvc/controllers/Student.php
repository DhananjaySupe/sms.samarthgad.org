	...
	class Student extends Admin_Controller {
		...
		*/
			...
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
				...
				if($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$array["aadhar_no"] = $this->input->post("aadhar_no");
						$array["saral_id_no"] = $this->input->post("saral_id_no");
		...
		public function export_student() {
			...
			$CONDITION = ['classesID' => $class_id];
			
			header('Content-Type: text/csv; charset=utf-8');  
			header('Content-Disposition: attachment; filename=data.csv');  
			$output = fopen("php://output", "w");  
			fputcsv($output, array('studentID', 'name', 'dob', 'admission_date', 'sex', 'religion', 'email', 'phone', 'address', 'classesID', 'sectionID', 'roll', 'bloodgroup', 'country', 'registerNO', 'state', 'library', 'hostel', 'transport', 'photo', 'parentID', 'createschoolyearID', 'schoolyearID', 'username', 'password', 'usertypeID', 'create_date', 'modify_date', 'create_userID', 'create_username', 'create_usertype', 'active', 'aadhar_no', 'saral_id_no'));  
			$result = $this->student_m->export_data($CONDITION);
			
			foreach ($result as $row) {
                //$present_days = $row->studentID;
				fputcsv($output, array($row->studentID, $row->name, $row->dob, $row->admission_date, $row->sex, $row->religion, $row->email, $row->phone, $row->address, $row->classesID, $row->sectionID, $row->roll, $row->bloodgroup, $row->country, $row->registerNO, $row->state, $row->library, $row->hostel, $row->transport, $row->photo, $row->parentID, $row->createschoolyearID, $row->schoolyearID, $row->username, $row->password, $row->usertypeID, $row->create_date, $row->modify_date, $row->create_userID, $row->create_username, $row->create_usertype, $row->active, $row->aadhar_no, $row->saral_id_no));  
			}
